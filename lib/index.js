...
class Builder {
  ...
  _initalizeModules() {
    for (let clzModule of this._clzComponent.__annotations.__class.Component.modules) {
  ...
  _inject(object) {
    if (!object.constructor.hasOwnProperty('__annotations')) {
      return;
    }
    let dependencies = [];
    for (let dependency of Object.keys(object.constructor.__annotations)) {
      if (object.constructor.__annotations[dependency].hasOwnProperty('Inject')) {
  ...
  _resolveDependencies() {
    for (let moduleName of Object.keys(this._modules)) {
      ...
      let annotations = module.constructor.__annotations;
  ...
  _verifyComponent() {
    try {
      if (this._clzComponent.__annotations.__class.Component != null) {
        return;
      }
    } catch (ignore) {}
...
module.exports = {
  setAutoProvideEnabled: function(autoProvideEnabled, rootDir) {
    // TODO
  }
};
